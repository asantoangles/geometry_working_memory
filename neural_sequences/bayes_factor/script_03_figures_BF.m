...
path_data = [path_root, '/results/neural_sequences/data'];
addpath([path_root, '/scripts/neural_sequences/utilities'])
...
neural_sequences_folders = {'sequence_concat_delay', 'sequence_concat_stim', 'sequence_trial_avg_delay', 'sequence_trial_avg_stim'};

% ATTENTION
neural_sequences_folders = {'sequence_concat_delay' 'sequence_trial_avg_delay'};
% ATTENTION


%% bayes factor

% loop over neural_sequences approaches
for folder_neural_sequences_i = 1:length(neural_sequences_folders)

    folder_neural_sequences = neural_sequences_folders{folder_neural_sequences_i};
    path_results = [path_root, '/results/neural_sequences/' folder_neural_sequences];

    if strcmp(folder_neural_sequences((end-4):end), 'delay')
    ...
    for folder_i = 1:length(folders)
        ...
        path_outputs = [path_root, '/results/neural_sequences/' folder_neural_sequences '/group_results/' classifier '/' folders{folder_i} '/bayes_factor'];
        ...
        for length_i = seq_lengths
            ...
            for fwd_bwd_i = 1:2 % 1:2 % ATTENTION
                ...
                disp(' ')
                disp('######################################################')
                disp([folder_neural_sequences ' - length ' num2str(length_i) ' - ' seq_name]);
                disp('######################################################')
