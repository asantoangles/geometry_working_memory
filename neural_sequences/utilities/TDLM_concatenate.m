function [Msf, Msb] = TDLM_concatenate(X,T,nstates,nlags,nshuf,X_trials)

% TDLM_concatenate estimates forward and backward 
% sequenceness in a decoded neural state space created through the
% concatenation of trials, while removing contamination across trials,
% using a two-step GLM approach based on Liu et al. (2019)
%
% [Msf, Msb] = TDLM_concatenate(X, T, nstates, nlags, nshuf, X_trials)
%
% Inputs:
...
%   X_trials - (subjects × sessions) number of trials per session per subject
%
% Outputs:
...
%
% Description:
%   This function computes sequenceness in a trial-concatenated decoded 
%   neural state space using a two-step GLM approach:
%     1) Constructs lagged predictors for each state and removes 
%        contamination across trial boundaries.
%     2) Fits a linear mixed-effects model (LME) for each state at each lag, 
%        accounting for random effects of subject, session, and trial.
%     3) Second-level regression of first-level coefficients onto 
%        transition template matrices to estimate forward and backward 
%        sequenceness uniquely attributable to hypothesized sequences.
%
% Notes:
%   - First permutation corresponds to the empirical transition matrix
%   - Automatically handles 3- or 4-state sequences
%   - Contaminated regions at trial boundaries are set to zero
% 
% REFERENCES:
%
%   Liu Y, Dolan RJ, Higgins C, Penagos H, Woolrich MW, Ólafsdóttir HF, Barry C, Kurth-Nelson Z, Behrens TE. 
%   Temporally delayed linear modelling (TDLM) measures neural_sequences in both animals and humans. Elife. 2021 Jun 7;10:e66917. 
...
trial_tp = size(X,1)/sum(X_trials,'all');
