...
for input_i = 1:length(inputs)
    ...
    for class_i = 1:length(classifiers)
        ...
        if ~isfile([path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier...
                    '/figure_stats_withingroup_balanced_perm' num2str(cfg_stat.n_permutations) '.jpeg'])
            ...
            if ~isfolder([path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier])
                mkdir([path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier]);
            end
    
            save([path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier...
                '/stats_withingroup_balanced_perm' num2str(cfg_stat.n_permutations) '.mat'], 'stat_level2');
            ...
            try
                ...
                saveas(gcf, [path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier...
                    '/figure_stats_withingroup_balanced_perm' num2str(cfg_stat.n_permutations) '.fig']);
                close all
            ...
            fig = openfig([path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier...
                    '/figure_stats_withingroup_balanced_perm' num2str(cfg_stat.n_permutations) '.fig']);
            saveas(fig, [path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier...
                    '/figure_stats_withingroup_balanced_perm' num2str(cfg_stat.n_permutations) '.jpeg']);
            close all
        ...
        if ~isfile([path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier...
            ...
            % plot average across locations
            close all
            imagesc(time, time, result_average.perf{1});
            set(gca, 'YDir', 'normal');
            colorbar; grid on;
            ylabel('Training time'), xlabel('Test time');
            title(['localizer ' classifiers{class_i} ' averaged locations / subjects']);
            saveas(gcf, [path_results '/group_results/localizer_twoclass/' inputs{input_i} '/' classifier...
