...
%% figure decoding localizer all classifiers
...
for input_i = 1:length(inputs)
    ...
    for class_i = 1:length(classifiers)
        ...
        plot(time, performance_allclassifiers(class_i,:), trace, 'Color', color, 'LineWidth', 2); hold on
    ...
    xlabel('time (ms)');
    title('Decoding of location during localizer');
    ylim([0.48 0.66]);

    saveas(gcf, [path_results '/group_results/localizer_twoclass/' inputs{input_i} '/figure_allclassifiers.fig']);
    fig = openfig([path_results '/group_results/localizer_twoclass/' inputs{input_i} '/figure_allclassifiers.fig']);
    saveas(fig, [path_results '/group_results/localizer_twoclass/' inputs{input_i} '/figure_allclassifiers.jpeg']);
    close all
