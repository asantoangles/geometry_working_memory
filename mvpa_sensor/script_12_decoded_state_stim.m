%% GERE project

% path to data
if isfolder('/path_to_local')
    path_preprocessed_data = '/path_to_local/results/preprocessing/M3';
    path_predictions = '/path_to_local/github/results/replay/data/predictions';
    path_results = '/path_to_local/results/mvpa/P1';
end

number_stimuli = 8;
classifiers = {'l2_e-1'};
inputs = {'bins'};
time_delay = 791; % bins
subjects = 1:17;
sessions = 1:2;

%% decoded state space (subject-specific) task_stim2task_stim

for sub_i = 1:length(subjects)

    subject = subjects(sub_i);

    % loop over sessions
    for ses_i = 1:length(sessions)

        session = sessions(ses_i);

        % set paths
        if subject < 10
            subject_ID = ['sub_0' num2str(subject)];
            subjectID = ['sub0' num2str(subject)];
        else
            subject_ID = ['sub_' num2str(subject)];
            subjectID = ['sub' num2str(subject)];
        end
        
        session_ID = ['sess_0' num2str(session)];

        disp([subject_ID ' - ' session_ID]); 
    
        % loop across classifiers
        for class_i = 1:length(classifiers)

            classifier = classifiers{class_i};

            for input_i = 1:length(inputs)

                if ~isfile([path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier '/predictions.mat'])

                    if ~isfolder([path_results '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier])
                        mkdir([path_results '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier]);
                    end

                    if ~isfolder([path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier])
                        mkdir([path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier]);
                    end
    
                    % average best time
                    best_times = [];
                    for stim_i = 1:number_stimuli
    
                        load([path_results '/' subject_ID '/' session_ID '/task_stim_balanced_iter1/' inputs{input_i} '/' classifier...
                                '/result_location' num2str(stim_i) '.mat']); % result
        
                        result = mv_select_result(result, 'auc');
            
                        [val, best_time] = max(result.perf, [], 1);
    
                        best_times = [best_times best_time];
    
                    end
    
                    best_time = round(median(best_times)); clear best_times
    
                    for stim_i = 1:number_stimuli
    
                        disp([classifier ' - ' inputs{input_i} ' - stimulus ' num2str(stim_i)])
                                                
                        %% prepare data - task stimulation (train)
            
                        % load
                        load([path_preprocessed_data '/' subject_ID '/' session_ID...
                                '/GERE_task_zthr25_wholetrial_baseline1sec_postICA.mat']); % data_task
    
                        data_task = data;
    
                        clear data
    
                        % subset correct trials
                        subset_trials = data_task.trialinfo(:,3);
                        subset_trials = find(subset_trials == 1);
                        data_task.trial = data_task.trial(subset_trials);
                        data_task.time = data_task.time(subset_trials);
                        data_task.sampleinfo = data_task.sampleinfo(subset_trials, :);
                        data_task.trialinfo = data_task.trialinfo(subset_trials, :);
                        
                        % X is variable with trial x channels x timepoints in task
                        % time_test is vector with the timepoints in task
                    
                        if strcmp(inputs{input_i}, 'bins')
                
                            % create bins
                            timepoints = 1700; % 100 ms baseline + (300 ms * 4 stimuli = 1200) + (100 ms * 3 isi = 300) + 100 ms delay
                            bin_length = 50;
                            bin_step = 5;
                            bin_number = round((timepoints) / bin_step) - (bin_step*2)+1;
                        
                            for bin_i = 1:bin_number
                                if bin_i == 1
                                    bins = 1:bin_length;
                                else
                                    bins = [bins; bins(end,:)+5];
                                end
                            end
                
                            % subset data_task
                            X = nan(size(data_task.trial, 2), size(data_task.label, 1), bin_number);
                            for trial_i = 1:size(data_task.trialinfo, 1)
                                all_timepoints = 901:2600;
                                for bin_i = 1:bin_number
                                    X(trial_i, :, bin_i) = mean(data_task.trial{trial_i}(:, all_timepoints(bins(bin_i,:))), 2)';
                                end
                            end
                            
                            % time
                            time = squeeze(mean(bins'));
        
                            % format X to accomodate order sequence
                            X_oldformat = X;
                            X = nan(size(X,1), size(X, 2), 91, 4); % third dimension: time comprising 100 ms isi + (300 ms stim * 4) + (100 ms isi * 3) + 100 ms delay
                                                                    % forth dimension: order sequence
            
                            for trial_i = 1:size(X, 1)
            
                                X(trial_i, :,:,1) = X_oldformat(trial_i, :, 1:91);          % 1:500 in timepoints
                                X(trial_i, :,:,2) = X_oldformat(trial_i, :, 81:171);        % 401:900 in timepoints
                                X(trial_i, :,:,3) = X_oldformat(trial_i, :, 161:251);       % 801:1300 in timepoints
            
                                if data_task.trialinfo(trial_i, 7) > 0
                                    X(trial_i, :,:,4) = X_oldformat(trial_i, :, 241:331); % 1201:1700 in timepoints
                                end
            
                            end
                        
                        end           
        
                        % subset X for trials of length 4
        
                        X_order4 = nan(0, size(X, 2), size(X, 3), 4);
        
                        for trial_i = 1:size(X, 1)
        
                            if data_task.trialinfo(trial_i, 7) > 0
        
                                X_order4(end+1,:,:,:) = X(trial_i,:,:,:);
        
                            end
        
                        end
        
                        % reformat X and clabel
        
                        X = [squeeze(X(:,:,:,1)); squeeze(X(:,:,:,2)); squeeze(X(:,:,:,3)); squeeze(X_order4(:,:,:,4))];
        
                        % set labels
                        clabel_train = data_task.trialinfo(:,4:7);
                        clabel_train = [clabel_train(:,1); clabel_train(:,2); clabel_train(:,3); clabel_train(:,4)];
                        clabel_train = clabel_train(clabel_train>0);
    
                        clabel_train(clabel_train == stim_i) = 100;
                        clabel_train(clabel_train < 100) = 2;
                        clabel_train(clabel_train == 100) = 1;
        
                        % subset best time
                        X_besttime_1tp = X(:,:,best_time);
                    
                        % set train data (repeating best time classifier)
                        X_train = nan(size(X_besttime_1tp, 1), size(X_besttime_1tp, 2), size(X, 3));
                        for time_i = 1:size(X_train, 3)
                            X_train(:,:,time_i) = X_besttime_1tp;
                        end

                        %% prepare data - task stimulation (test)

                        % load
                        load([path_preprocessed_data '/' subject_ID '/' session_ID...
                                '/GERE_task_zthr25_wholetrial_baseline1sec_postICA.mat']); % data_task
    
                        data_task = data;
    
                        clear data
    
                        % subset correct trials
                        subset_trials = data_task.trialinfo(:,3);
                        subset_trials = find(subset_trials == 1);
                        data_task.trial = data_task.trial(subset_trials);
                        data_task.time = data_task.time(subset_trials);
                        data_task.sampleinfo = data_task.sampleinfo(subset_trials, :);
                        data_task.trialinfo = data_task.trialinfo(subset_trials, :);
                        
                        % X is variable with trial x channels x timepoints in task
                        % time_test is vector with the timepoints in task
                    
                        if strcmp(inputs{input_i}, 'bins')
                
                            % create bins
                            timepoints = 1700; % 100 ms baseline + (300 ms * 4 stimuli = 1200) + (100 ms * 3 isi = 300) + 100 ms delay
                            bin_length = 50;
                            bin_step = 5;
                            bin_number = round((timepoints) / bin_step) - (bin_step*2)+1;
                        
                            for bin_i = 1:bin_number
                                if bin_i == 1
                                    bins = 1:bin_length;
                                else
                                    bins = [bins; bins(end,:)+5];
                                end
                            end
                
                            % subset data_task
                            X = nan(size(data_task.trial, 2), size(data_task.label, 1), bin_number);
                            for trial_i = 1:size(data_task.trialinfo, 1)
                                all_timepoints = 901:2600;
                                for bin_i = 1:bin_number
                                    X(trial_i, :, bin_i) = mean(data_task.trial{trial_i}(:, all_timepoints(bins(bin_i,:))), 2)';
                                end
                            end
                            
                            % time
                            time = squeeze(mean(bins'));
        
                            % format X to accomodate order sequence
                            X_oldformat = X;
                            X = nan(size(X,1), size(X, 2), 91, 4); % third dimension: time comprising 100 ms isi + (300 ms stim * 4) + (100 ms isi * 3) + 100 ms delay
                                                                    % forth dimension: order sequence
            
                            for trial_i = 1:size(X, 1)
            
                                X(trial_i, :,:,1) = X_oldformat(trial_i, :, 1:91);          % 1:500 in timepoints
                                X(trial_i, :,:,2) = X_oldformat(trial_i, :, 81:171);        % 401:900 in timepoints
                                X(trial_i, :,:,3) = X_oldformat(trial_i, :, 161:251);       % 801:1300 in timepoints
                                X(trial_i, :,:,4) = X_oldformat(trial_i, :, 241:331); % 1201:1700 in timepoints
            
                            end
                        
                        end           
                
                        % reformat X and clabel
        
                        X_test = [squeeze(X(:,:,:,1)); squeeze(X(:,:,:,2)); squeeze(X(:,:,:,3)); squeeze(X(:,:,:,4))];
        
                        % set labels
                        clabel_test = data_task.trialinfo(:,4:7);
                        clabel_test = [clabel_test(:,1); clabel_test(:,2); clabel_test(:,3); clabel_test(:,4)];
                        clabel_test(clabel_test>0) = 9;
    
                        clabel_test(clabel_test == stim_i) = 100;
                        clabel_test(clabel_test < 100) = 2;
                        clabel_test(clabel_test == 100) = 1;
        
                                    
                        %% compute weights
                        cfg                         = [];
                        if strcmp(classifier, 'l2_e-1')
                            cfg.classifier              = 'logreg';
                            cfg.hyperparameter.reg      = 'l2';
                            cfg.hyperparameter.lambda      = 0.1;
                        end
                        cfg.metric                  = {'dval'};
                        cfg.preprocess              = {'zscore'}; % nested preprocessing
                        cfg.save                    = {'model_param'};
                        [~, result, ~]   = mv_classify(cfg, X_train(:,:,1), clabel_train, X_test(1,:,1), clabel_test(1));
    
                        cf = result.model_param{1};

                        save([path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier...
                                '/weights_location' num2str(stim_i) '.mat'], 'cf', '-v7.3');
    
                        %% preprocessing X_test
    
                        pparam = mv_get_preprocess_param('zscore');
                        [~, X_preproc] = mv_preprocess_zscore(pparam, X_test);
    
                        %% compute predictions
    
                        dval_all = nan(size(X_preproc, 1), size(X_preproc, 3));
    
                        for trial_i = 1:size(dval_all,1)
                            
                            %% single trial predictions
                        
                            for time_i = 1:size(X_preproc, 3)
            
                                X_trial = squeeze(X_preproc(trial_i, :, time_i));
    
                                if strcmp(classifier, 'svm')
    
                                    [~,dval] = test_svm(cf, X_trial);
    
                                else
    
                                    [~,dval] = test_logreg(cf, X_trial);
    
                                end
    
                                dval_all(trial_i, time_i) = dval;
    
                            end
                                            
                        end
                                    
                        predictions_stim = dval_all;
    
                        filename_output = [path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier...
                                '/predictions_trial_predictions_dval_stim' num2str(stim_i) '.mat'];
        
                        save(filename_output, 'predictions_stim', '-v7.3');
            
                    end
    
                    % merge predictions
                    predictions = [];
                    predictions.info = {'dimensions: trial / time (delay) / stimuli'};
                    predictions.dval = nan(size(X_test, 1), size(X_test, 3), number_stimuli);
    
                    for stim_i = 1:number_stimuli
    
                        filename_output = [path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier...
                                '/predictions_trial_predictions_dval_stim' num2str(stim_i) '.mat'];
    
                        load(filename_output)
    
                        predictions.dval(:,:,stim_i) = predictions_stim;
    
                    end
        
                    system(['rm ' path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier...
                                '/predictions_trial_predictions_dval_stim*']);

                    % reset predictions
                    predictions_old = predictions;
                    number_trials = size(data_task.trial, 2);
                    predictions.dval = [predictions_old.dval(1:number_trials,:,:) predictions_old.dval((number_trials+1):(2*number_trials),:,:)];
                    predictions.dval = [predictions.dval predictions_old.dval(((2*number_trials)+1):(3*number_trials),:,:)];
                    predictions.dval = [predictions.dval predictions_old.dval(((3*number_trials)+1):(4*number_trials),:,:)];

                    
                    % delete repeated ISI intervals
                    if strcmp(inputs{input_i}, 'bins')
                        predictions.dval = predictions.dval(:,[1:80 91:160 171:240 251:330],:);
                    end

                    save([path_predictions '/' subject_ID '/' session_ID '/task_stim2task_stim/' inputs{input_i} '/' classifier '/predictions.mat'], 'predictions', '-v7.3');



                end

            end
            
        end
        
    end

end


%% decoded state space (subject-specific) localizer2task_stim

for sub_i = 1:length(subjects)

    subject = subjects(sub_i);

    % loop over sessions
    for ses_i = 1:length(sessions)

        session = sessions(ses_i);

        % set paths
        if subject < 10
            subject_ID = ['sub_0' num2str(subject)];
            subjectID = ['sub0' num2str(subject)];
        else
            subject_ID = ['sub_' num2str(subject)];
            subjectID = ['sub' num2str(subject)];
        end
        
        session_ID = ['sess_0' num2str(session)];

        disp([subject_ID ' - ' session_ID]); 
    
        % loop across classifiers
        for class_i = 1:length(classifiers)

            classifier = classifiers{class_i};

            for input_i = 1:length(inputs)

                if ~isfile([path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier '/predictions.mat'])

                    if ~isfolder([path_results '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier])
                        mkdir([path_results '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier]);
                    end

                    if ~isfolder([path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier])
                        mkdir([path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier]);
                    end
    
                    % average best time
                    best_times = [];
                    for stim_i = 1:number_stimuli
    
                        load([path_results '/' subject_ID '/' session_ID '/localizer_twoclass/' inputs{input_i} '/' classifier...
                                '/result_balanced_location' num2str(stim_i) '.mat']); % result
        
                        result = mv_select_result(result, 'auc');
            
                        [val, best_time] = max(result.perf, [], 1);
    
                        best_times = [best_times best_time];
    
                    end
    
                    best_time = round(median(best_times)); clear best_times
    
                    for stim_i = 1:number_stimuli
    
                        disp([classifier ' - ' inputs{input_i} ' - stimulus ' num2str(stim_i)])
                                                
                        %% prepare data - task stimulation (train)
            
                        % load
                        load([path_preprocessed_data '/' subject_ID '/' session_ID...
                                '/GERE_task_zthr25_wholetrial_baseline1sec_postICA.mat']); % data_task
        
                        data_task = data;
        
                        clear data
    
                        % subset correct trials
                        subset_trials = data_task.trialinfo(:,3);
                        subset_trials = find(subset_trials == 1);
                        data_task.trial = data_task.trial(subset_trials);
                        data_task.time = data_task.time(subset_trials);
                        data_task.sampleinfo = data_task.sampleinfo(subset_trials, :);
                        data_task.trialinfo = data_task.trialinfo(subset_trials, :);
                        
                        % X is variable with trial x channels x timepoints in task
                        % time_test is vector with the timepoints in task
                    
                        if strcmp(inputs{input_i}, 'bins')
                
                            % create bins
                            timepoints = 1700; % 100 ms baseline + (300 ms * 4 stimuli = 1200) + (100 ms * 3 isi = 300) + 100 ms delay
                            bin_length = 50;
                            bin_step = 5;
                            bin_number = round((timepoints) / bin_step) - (bin_step*2)+1;
                        
                            for bin_i = 1:bin_number
                                if bin_i == 1
                                    bins = 1:bin_length;
                                else
                                    bins = [bins; bins(end,:)+5];
                                end
                            end
                
                            % subset data_task
                            X = nan(size(data_task.trial, 2), size(data_task.label, 1), bin_number);
                            for trial_i = 1:size(data_task.trialinfo, 1)
                                all_timepoints = 901:2600;
                                for bin_i = 1:bin_number
                                    X(trial_i, :, bin_i) = mean(data_task.trial{trial_i}(:, all_timepoints(bins(bin_i,:))), 2)';
                                end
                            end
                            
                            % time
                            time = squeeze(mean(bins'));
        
                            % format X to accomodate order sequence
                            X_oldformat = X;
                            X = nan(size(X,1), size(X, 2), 91, 4); % third dimension: time comprising 100 ms isi + (300 ms stim * 4) + (100 ms isi * 3) + 100 ms delay
                                                                    % forth dimension: order sequence
            
                            for trial_i = 1:size(X, 1)
            
                                X(trial_i, :,:,1) = X_oldformat(trial_i, :, 1:91);          % 1:500 in timepoints
                                X(trial_i, :,:,2) = X_oldformat(trial_i, :, 81:171);        % 401:900 in timepoints
                                X(trial_i, :,:,3) = X_oldformat(trial_i, :, 161:251);       % 801:1300 in timepoints
            
                                if data_task.trialinfo(trial_i, 7) > 0
                                    X(trial_i, :,:,4) = X_oldformat(trial_i, :, 241:331); % 1201:1700 in timepoints
                                end
            
                            end
                        
                        end           
        
                        % subset X for trials of length 4
        
                        X_order4 = nan(0, size(X, 2), size(X, 3), 4);
        
                        for trial_i = 1:size(X, 1)
        
                            if data_task.trialinfo(trial_i, 7) > 0
        
                                X_order4(end+1,:,:,:) = X(trial_i,:,:,:);
        
                            end
        
                        end
        
                        % reformat X and clabel
        
                        X = [squeeze(X(:,:,:,1)); squeeze(X(:,:,:,2)); squeeze(X(:,:,:,3)); squeeze(X_order4(:,:,:,4))];
        
                        % set labels
                        clabel_train = data_task.trialinfo(:,4:7);
                        clabel_train = [clabel_train(:,1); clabel_train(:,2); clabel_train(:,3); clabel_train(:,4)];
                        clabel_train = clabel_train(clabel_train>0);
    
                        clabel_train(clabel_train == stim_i) = 100;
                        clabel_train(clabel_train < 100) = 2;
                        clabel_train(clabel_train == 100) = 1;
        
                        % subset best time
                        X_besttime_1tp = X(:,:,best_time);
                    
                        % set train data (repeating best time classifier)
                        X_train = nan(size(X_besttime_1tp, 1), size(X_besttime_1tp, 2), size(X, 3));
                        for time_i = 1:size(X_train, 3)
                            X_train(:,:,time_i) = X_besttime_1tp;
                        end

                        %% prepare data - task stimulation (test)

                        % load
                        load([path_preprocessed_data '/' subject_ID '/' session_ID...
                                '/GERE_task_zthr25_wholetrial_baseline1sec_postICA.mat']); % data_task
        
                        data_task = data;
        
                        clear data
    
                        % subset correct trials
                        subset_trials = data_task.trialinfo(:,3);
                        subset_trials = find(subset_trials == 1);
                        data_task.trial = data_task.trial(subset_trials);
                        data_task.time = data_task.time(subset_trials);
                        data_task.sampleinfo = data_task.sampleinfo(subset_trials, :);
                        data_task.trialinfo = data_task.trialinfo(subset_trials, :);
                        
                        % X is variable with trial x channels x timepoints in task
                        % time_test is vector with the timepoints in task
                    
                        if strcmp(inputs{input_i}, 'bins')
                
                            % create bins
                            timepoints = 1700; % 100 ms baseline + (300 ms * 4 stimuli = 1200) + (100 ms * 3 isi = 300) + 100 ms delay
                            bin_length = 50;
                            bin_step = 5;
                            bin_number = round((timepoints) / bin_step) - (bin_step*2)+1;
                        
                            for bin_i = 1:bin_number
                                if bin_i == 1
                                    bins = 1:bin_length;
                                else
                                    bins = [bins; bins(end,:)+5];
                                end
                            end
                
                            % subset data_task
                            X = nan(size(data_task.trial, 2), size(data_task.label, 1), bin_number);
                            for trial_i = 1:size(data_task.trialinfo, 1)
                                all_timepoints = 901:2600;
                                for bin_i = 1:bin_number
                                    X(trial_i, :, bin_i) = mean(data_task.trial{trial_i}(:, all_timepoints(bins(bin_i,:))), 2)';
                                end
                            end
                            
                            % time
                            time = squeeze(mean(bins'));
        
                            % format X to accomodate order sequence
                            X_oldformat = X;
                            X = nan(size(X,1), size(X, 2), 91, 4); % third dimension: time comprising 100 ms isi + (300 ms stim * 4) + (100 ms isi * 3) + 100 ms delay
                                                                    % forth dimension: order sequence
            
                            for trial_i = 1:size(X, 1)
            
                                X(trial_i, :,:,1) = X_oldformat(trial_i, :, 1:91);          % 1:500 in timepoints
                                X(trial_i, :,:,2) = X_oldformat(trial_i, :, 81:171);        % 401:900 in timepoints
                                X(trial_i, :,:,3) = X_oldformat(trial_i, :, 161:251);       % 801:1300 in timepoints
                                X(trial_i, :,:,4) = X_oldformat(trial_i, :, 241:331); % 1201:1700 in timepoints
            
                            end
        
                        end           
                
                        % reformat X and clabel
        
                        X_test = [squeeze(X(:,:,:,1)); squeeze(X(:,:,:,2)); squeeze(X(:,:,:,3)); squeeze(X(:,:,:,4))];
        
                        % set labels
                        clabel_test = data_task.trialinfo(:,4:7);
                        clabel_test = [clabel_test(:,1); clabel_test(:,2); clabel_test(:,3); clabel_test(:,4)];
                        clabel_test(clabel_test>0) = 9;
    
                        clabel_test(clabel_test == stim_i) = 100;
                        clabel_test(clabel_test < 100) = 2;
                        clabel_test(clabel_test == 100) = 1;
        
                                    
                        %% compute weights
                        cfg                         = [];
                        if strcmp(classifier, 'l2_e-1')
                            cfg.classifier              = 'logreg';
                            cfg.hyperparameter.reg      = 'l2';
                            cfg.hyperparameter.lambda      = 0.1;
                        end
                        cfg.metric                  = {'dval'};
                        cfg.preprocess              = {'zscore'}; % nested preprocessing
                        cfg.save                    = {'model_param'};
                        [~, result, ~]   = mv_classify(cfg, X_train(:,:,1), clabel_train, X_test(1,:,1), clabel_test(1));
    
                        cf = result.model_param{1};

                        save([path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier...
                                '/weights_location' num2str(stim_i) '.mat'], 'cf', '-v7.3');
    
                        %% preprocessing X_test
    
                        pparam = mv_get_preprocess_param('zscore');
                        [~, X_preproc] = mv_preprocess_zscore(pparam, X_test);
    
                        %% compute predictions
    
                        dval_all = nan(size(X_preproc, 1), size(X_preproc, 3));
    
                        for trial_i = 1:size(dval_all,1)
                            
                            %% single trial predictions
                        
                            for time_i = 1:size(X_preproc, 3)
            
                                X_trial = squeeze(X_preproc(trial_i, :, time_i));
    
                                if strcmp(classifier, 'svm')
    
                                    [~,dval] = test_svm(cf, X_trial);
    
                                else
    
                                    [~,dval] = test_logreg(cf, X_trial);
    
                                end
    
                                dval_all(trial_i, time_i) = dval;
    
                            end
                                            
                        end
                                    
                        predictions_stim = dval_all;
    
                        filename_output = [path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier...
                                '/predictions_trial_predictions_dval_stim' num2str(stim_i) '.mat'];
        
                        save(filename_output, 'predictions_stim', '-v7.3');
            
                    end
    
                    % merge predictions
                    predictions = [];
                    predictions.info = {'dimensions: trial / time (delay) / stimuli'};
                    predictions.dval = nan(size(X_test, 1), size(X_test, 3), number_stimuli);
    
                    for stim_i = 1:number_stimuli
    
                        filename_output = [path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier...
                                '/predictions_trial_predictions_dval_stim' num2str(stim_i) '.mat'];
    
                        load(filename_output)
    
                        predictions.dval(:,:,stim_i) = predictions_stim;
    
                    end
        
                    system(['rm ' path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier...
                                '/predictions_trial_predictions_dval_stim*']);

                    % reset predictions
                    predictions_old = predictions;
                    number_trials = size(data_task.trial, 2);
                    predictions.dval = [predictions_old.dval(1:number_trials,:,:) predictions_old.dval((number_trials+1):(2*number_trials),:,:)];
                    predictions.dval = [predictions.dval predictions_old.dval(((2*number_trials)+1):(3*number_trials),:,:)];
                    predictions.dval = [predictions.dval predictions_old.dval(((3*number_trials)+1):(4*number_trials),:,:)];

                    
                    % delete repeated ISI intervals
                    if strcmp(inputs{input_i}, 'bins')
                        predictions.dval = predictions.dval(:,[1:80 91:160 171:240 251:330],:);
                    end

                    save([path_predictions '/' subject_ID '/' session_ID '/localizer2task_stim/' inputs{input_i} '/' classifier '/predictions.mat'], 'predictions', '-v7.3');

                end

            end
            
        end
        
    end

end


disp('Analysis done');
