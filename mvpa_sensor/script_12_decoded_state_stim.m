...
%% compute decoded state space (subject-specific) for the stimulus period
    ...
    for ses_i = 1:length(sessions)
        ...
        for class_i = 1:length(classifiers)
            ...
            for input_i = 1:length(inputs)
                ...
                if ~isfile([path_results '/' subject_ID '/' session_ID '/task_stim_balanced2task_stim_trial_predictions_averaged/' inputs{input_i} '/' classifier...
                                '/predictions_singletrial.mat'])

                    if ~isfolder([path_results '/' subject_ID '/' session_ID '/task_stim_balanced2task_stim_trial_predictions_averaged/' inputs{input_i} '/' classifier])
                        mkdir([path_results '/' subject_ID '/' session_ID '/task_stim_balanced2task_stim_trial_predictions_averaged/' inputs{input_i} '/' classifier]);
                    end
                    ...
                    for stim_i = 1:number_stimuli
                        ...
                        elseif strcmp(inputs{input_i}, 'timepoints')
                            ...
                            for trial_i = 1:size(data_task.trial, 2)
                                data_task.time{trial_i} = data_task.time{trial_i}(1, 901:2600);
                                data_task.trial{trial_i} = data_task.trial{trial_i}(:, 901:2600);
                            end
                
                            data_task.sampleinfo(:,1) = data_task.sampleinfo(:,1) + 800;
                            data_task.sampleinfo(:,2) = data_task.sampleinfo(:,1) + 1700;
                            ...
                            % format X to accomodate order sequence
                            X_oldformat = X;
                            X = nan(size(X,1), size(X, 2), 500, 4); % third dimension: time comprising 100 ms isi + 300 ms stim + 100 ms isi
                                                                    % forth dimension: order sequence
            
                            for trial_i = 1:size(X, 1)
            
                                X(trial_i, :,:,1) = X_oldformat(trial_i, :, 1:500);
                                X(trial_i, :,:,2) = X_oldformat(trial_i, :, 401:900);
                                X(trial_i, :,:,3) = X_oldformat(trial_i, :, 801:1300);
                                X(trial_i, :,:,4) = X_oldformat(trial_i, :, 1201:1700);
                        ...
                        save([path_results '/' subject_ID '/' session_ID '/task_stim_balanced2task_stim_trial_predictions_averaged/' inputs{input_i} '/' classifier...
                        ...
                        filename_output = [path_results '/' subject_ID '/' session_ID '/task_stim_balanced2task_stim_trial_predictions_averaged/' inputs{input_i} '/' classifier...
                    ...
                    system(['rm ' path_results '/' subject_ID '/' session_ID '/task_stim_balanced2task_stim_trial_predictions_averaged/' inputs{input_i} '/' classifier...
                    ...
                    if strcmp(inputs{input_i}, 'timepoints')
                        predictions.dval = predictions.dval(:,[101:400 501:800 901:1200 1301:1600],:);
