                    ...
                    if ~isempty(X)
                        ...
                        if ~isfile([outdir '/vaf_' seq_name '.mat'])
                            ...
                            if strcmp(events{event_i}, 'delay')
                                ...
                                [z_k, z_k_exp, planes] = compute_subspaces(X, X, components, number_locations);
    
                            elseif strcmp(events{event_i}, 'stim')
    
                                [z_k, z_k_exp, planes] = compute_subspaces_stim(X, X, components, stim_order, number_locations);
                            ...
                            if ~isfile([outdir '/vaf_bootstrap_' seq_name '.mat'])
                                ...
                                for iter_i = 1:iterations_bootstrap
                                    ...
                                    if strcmp(events{event_i}, 'delay')
                                        ...
                                        [planes_split1, planes_split2, Y_r, score] = compute_subspaces_within(X, X_boostrap.X_split1(:,:,iter_i), X_boostrap.X_split2(:,:,iter_i), components, number_locations);
            
                                    elseif strcmp(events{event_i}, 'stim')
            
                                        [planes_split1, planes_split2, Y_r, score] = compute_subspaces_within_stim(X, X_boostrap.X_split1(:,:,iter_i), X_boostrap.X_split2(:,:,iter_i), components, stim_order);
