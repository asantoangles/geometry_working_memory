...
for (separability_metric in separability_metrics) {
  ...
  for (fun in functions_data) {
    ...
    filename = paste(path, "/", fun, "/correct_trials/design_matrix/stim_length", length_seq, "_1to300.txt", sep = "")
    design_matrix_correct = read.table(filename, sep = ",", header = TRUE)
    design_matrix_correct$performance = "correct"
    filename = paste(path, "/", fun, "/incorrect_trials/design_matrix/stim_length", length_seq, "_1to300.txt", sep = "")
