...
for (separability in separability_metrics) {
  ...
  path_random = paste(path_results_random, "/group_results/", separability, sep = "")
  
  cat(separability); cat(' ')
  
  if (separability == "distance") {
    p_values = p_values_all[1:33]
    p_values_random = p_values_all_random[1:33]
    y_axis_step = 0.05
  }
  if (separability == "volume") {
    p_values = p_values_all[34:66]
    p_values_random = p_values_all_random[34:66]
    y_axis_step = 0.005
  }
  ...
  for (fun in functions_data) {
    ...
    incorrect_avg = c()
    incorrect_sd = c()
    correct_random_avg = c()
    correct_random_sd = c()
    incorrect_random_avg = c()
    incorrect_random_sd = c()
    ...
    correct_random_se = correct_random_sd / sqrt(15)
    incorrect_random_se = incorrect_random_sd / sqrt(15)
    ...
    ylim_metric = c(min(data$avg - data$se), max(data$avg + data$se) + 0.02 * max(data$avg + data$se))
