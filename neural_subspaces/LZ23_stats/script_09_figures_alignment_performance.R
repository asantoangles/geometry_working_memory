...
for (metric_i in metrics) {
  ...
  for (fun in functions_data) {
    ...
    y_pvalue = max(c(data$avg + data$se)) + (max(c(data$avg + data$se))/200)
    ...
    ylim_metric = c(min(data$avg - data$se), max(data$avg + data$se) + 0.05 * max(data$avg + data$se))
    ...
    plot = 
      ...
      scale_y_continuous(
        limits = ylim_metric,
        breaks = seq(
        floor(min(data$avg - data$se) / y_axis_step) * y_axis_step,
        ceiling(max(data$avg + data$se) / y_axis_step) * y_axis_step,
        by = y_axis_step
        )
      ) + 
