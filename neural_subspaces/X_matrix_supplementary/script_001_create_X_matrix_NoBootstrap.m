...
folder_outputs = 'X_matrix_supplementary';
        ...
        for perf_i = 1:length(performance)
            ...
            for seg_i = 1:size(time_segments, 1)
                ...
                for sequence_length = [3 4]
                    ...
                    data_task_beforesubset = data_task;
                    data_baseline_beforesubset = data_baseline;
        
                    %% X matrix

                    for supp_i = 1:5

                        data_task = data_task_beforesubset;
                        data_baseline = data_baseline_beforesubset;

                        % idx of subset trials
                        number_trials = size(data_task.trial,2);
                        total_trials_subset = round(supp_i * (number_trials / 5));
                        if supp_i == 5
                            total_trials_subset = number_trials;
                        end
                        idx_subset_trials = 1:total_trials_subset;

                        % subset data_task and data_baseline

                        data_task.trial     = data_task.trial(idx_subset_trials);
                        data_task.time      = data_task.time(idx_subset_trials);
                        data_task.trialinfo = data_task.trialinfo(idx_subset_trials,:);
                        
                        data_baseline.trial     = data_baseline.trial(idx_subset_trials);
                        data_baseline.time      = data_baseline.time(idx_subset_trials);
                        data_baseline.trialinfo = data_baseline.trialinfo(idx_subset_trials,:);

                        % Create output directory
                        path_results_supp_i = [path_results '_supp' num2str(supp_i)];
                        outdir = [path_results_supp_i '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i} '/' ...
                            events{event_i} '_' num2str(seg_start) 'to' num2str(seg_end)];
