...
folder_outputs = 'LZ23_controlled_resampling';
...
path_inputs = [path_root '/results/neural_subspaces/' folder_inputs];
path_results = [path_root '/results/neural_subspaces/' folder_outputs];
addpath([path_root '/scripts/neural_subspaces/utilities'])

folder_X_matrix = 'X_matrix';
path_X_matrix = [path_root '/results/neural_subspaces/' folder_X_matrix];

folder_original = 'LZ23';
path_inputs_original = [path_root '/results/neural_subspaces/' folder_original];
            ...
            for perf_i = 1:length(performance)
                ...
                for sequence_length = [3 4]
                    ...
                    if ~isfile([outdir '/separability_perm_' seq_name '.mat'])
                        ...
                        angle_min_perm = nan(stim_order,stim_order,iterations_bootstrap);
                            ...
                            if ~isfile([outdir '/separability_perm_' seq_name '.mat'])
                                ...
                                if ~isempty(X_resampling)
                                    ...
                                    for iter_i = 1:iterations_bootstrap
                                        ...
                                        [angle_perm(:,:,iter_i), vaf_perm(:,:,iter_i), angle_min_perm(:,:,iter_i)] = compute_alignment_metrics(planes, stim_order);
                                        ...
                                        for rank_i = 1:stim_order
                                            ...
                                            [separability.volume{rank_i}, separability.distance_by_separation{rank_i}] = compute_separability_metrics_highdim(points);
                                            
                                            % set optimal length
                                            if isempty(separability.volume{rank_i})
                                                separability.volume{rank_i} = 0;
                                            end
            
                                            % set optimal length
                                            vec = separability.distance_by_separation{rank_i};
                                            target_len = number_locations - 1;
                                            if size(vec, 2) ~= target_len
                                                % Pad with zeros
                                                if size(vec, 2) < target_len
                                                    vec = [vec, zeros(1, target_len - size(vec, 2))];
                                                else
                                                    vec = vec(1:target_len);
                                                end
                                                separability.distance_by_separation{rank_i} = vec;
                                            end
                                    ...
                                    save([outdir '/angle_min_perm_' seq_name '.mat'], 'angle_min_perm');
                ...
                for sequence_length = [3 4]
                    ...
                    if ~isempty(X)
                        ...
                        if strcmp(performance{perf_i}, 'correct_trials')
                            ...
                            %% angle_min
                            load([outdir '/angle_min_perm_' seq_name '.mat']); % angle_min_perm
    
                            angle_min = mean(angle_min_perm,3);
    
                            save([outdir '/angle_min_' seq_name '.mat'], 'angle_min');
                        ...
                        elseif strcmp(performance{perf_i}, 'incorrect_trials')
                            ...
                            source = [indir_original '/angle_min_' seq_name '.mat'];
                            target = [outdir '/angle_min_' seq_name '.mat'];
                            system(['cp ' source ' ' target]);
