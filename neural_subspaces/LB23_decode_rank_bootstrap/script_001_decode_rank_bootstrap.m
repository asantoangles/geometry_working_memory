...
path_results_template = [path_root '/results/neural_subspaces/X_matrix'];
                            ...
                            for train_test_i = 1:2
                                ...
                                for perm_i = 1:size(X_allperm, 3)
                                    ...
                                    if strcmp(events{event_i}, 'delay')
                                        [z_k, ~, ~] = compute_subspaces(X, X, components, number_locations);
                                    elseif strcmp(events{event_i}, 'stim')
                                        [z_k, ~, ~] = compute_subspaces_stim(X, X, components, stim_order, number_locations);
                                    end
