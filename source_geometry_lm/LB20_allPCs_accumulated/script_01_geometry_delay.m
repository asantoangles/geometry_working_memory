    ...
    for event_i = 1:length(events)
        ...
        for sub_i = 1:length(subjects)
            ...
            for perf_i = 1:length(performance)
                ...
                if strcmp(events{event_i}, 'stim_resolved') % same than stim
                ...
                elseif strcmp(events{event_i}, 'delay')
            ...
            for fun_i = functions_data
                ...
                for delay_i = 1:size(time_segments, 1)
                    ...
                    %% compute X matrix with ridge regression model - correct trials
                    ...
                    for sequence_length = [3 4 34]
                        ...
                        if ~isfolder([path_results '/' subject_ID '/' func2str(fun_i) '/correct_trials/' ...
                                events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))])
                            mkdir([path_results '/' subject_ID '/' func2str(fun_i) '/correct_trials/' ...
                                events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))]);
                        end
                    
                        save([path_results '/' subject_ID '/' func2str(fun_i) '/correct_trials/' ...
                                events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2)) ...
                    ...
                    for perf_i = 1:length(performance)
                        ...
                        for sequence_length = [3 4 34]
                            ...
                            % eigenvectors
                            X_correct_trials = load([path_results '/' subject_ID '/' func2str(fun_i) '/correct_trials/' ...
                                events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2)) ...
                                '/X_matrix_' sequence_length_filename '.mat']);
                            X_correct_trials = X_correct_trials.X;
    
                            X_before_random = X;
                            X_correct_trials_before_random = X_correct_trials;
                            ...
                            if ~isfile([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                ...
                                [angle_min, angle, vaf] = compute_angle_vaf_accumulated(planes, ranks);
                            ...
                            if ~isfile([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                ...
                                angle_bootstrap_alliter = nan(ranks, ranks, iterations_bootstrap);
                                angle_min_bootstrap_alliter = nan(ranks, ranks, iterations_bootstrap);
                                vaf_bootstrap_alliter = nan(ranks, ranks, iterations_bootstrap);
                                ...
                                for iter_i = 1:iterations_bootstrap
                                    ...
                                    [planes_split1, planes_split2, Y_r, score] = compute_plane_components_within_alltrials(X_correct_trials, X_split1, X_split2, components);
        
                                    % compute angle and vaf within
                                    [angle_min_bootstrap_alliter(:,:,iter_i), angle_bootstrap_alliter(:,:,iter_i), vaf_bootstrap_alliter(:,:,iter_i)] = compute_angle_vaf_accumulated_within(planes_split1, planes_split2, Y_r, score, ranks);
