                ...
                for delay_i = 1:size(time_segments, 1)
                    ...
                    for perf_i = 1:length(performance)
                        ...
                        for sequence_length = [3 4 34]
                            ...
                            components = 1:last_comp_i;
    
                            if ~isfolder([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i} '/' ...
                                    events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))])
                                mkdir([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i} '/' ...
                                    events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))]);
                            end
    
                            %% compute PCA, best-fitting planes, principal angles and variance metrics
            
                            if ~isfile([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                    '/' events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))...
                                ...
                                [angle_min, angle, vaf] = compute_angle_vaf_accumulated(planes, stim_order);
                                ...
                                save([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                    '/' events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))...
                            ...
                            if ~isfile([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                '/' events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))...
                                '/___separability_' sequence_length_filename '.mat'])
        
                                load([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                    '/' events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))...
                                ...
                                for rank_i = 1:stim_order
                                    ...
                                    points = zscore(low_dim_space.z_k(rank, 1:last_comp_i));
        
                                    [separability.volume{rank_i}, separability.distance_by_separation{rank_i}] = compute_distance_accum(points);
                            ...
                            if ~isfile([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                ...
                                angle_min_bootstrap_alliter = nan(stim_order, stim_order, iterations_bootstrap);
                                ...
                                for iter_i = 1:iterations_bootstrap
                                    ...
                                    [angle_min_bootstrap_alliter(:,:,iter_i), angle_bootstrap_alliter(:,:,iter_i), vaf_bootstrap_alliter(:,:,iter_i)] = compute_angle_vaf_accumulated_within(planes_split1, planes_split2, Y_r, score, stim_order);
                                ...
                                angle_bootstrap.upper_ci = median(angle_bootstrap_alliter,3) + z * std(angle_bootstrap_alliter,1,3) / sqrt(iterations_bootstrap);
                                angle_bootstrap.lower_ci = median(angle_bootstrap_alliter,3) - z * std(angle_bootstrap_alliter,1,3) / sqrt(iterations_bootstrap);
                                ...
                                angle_min_bootstrap = [];
                                angle_min_bootstrap.median = median(angle_min_bootstrap_alliter,3);
                                angle_min_bootstrap.std = std(angle_min_bootstrap_alliter,1,3);
                                angle_min_bootstrap.upper_ci = median(angle_min_bootstrap_alliter,3) + z * std(angle_min_bootstrap_alliter,1,3) / sqrt(iterations_bootstrap);
                                angle_min_bootstrap.lower_ci = median(angle_min_bootstrap_alliter,3) - z * std(angle_min_bootstrap_alliter,1,3) / sqrt(iterations_bootstrap);
        
                                save([path_results '/comp' num2str(components(1)) 'to' num2str(components(end)) '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                    '/' events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))...
                                    '/angle_min_bootstrap_' sequence_length_filename '.mat'], 'angle_min_bootstrap', '-v7.3');
                                
                                vaf_bootstrap = [];
                                vaf_bootstrap.median = median(vaf_bootstrap_alliter,3);
                                vaf_bootstrap.std = std(vaf_bootstrap_alliter,1,3);
                                vaf_bootstrap.upper_ci = median(vaf_bootstrap_alliter,3) - z * std(vaf_bootstrap_alliter,1,3) / sqrt(iterations_bootstrap);
                                vaf_bootstrap.lower_ci = median(vaf_bootstrap_alliter,3) + z * std(vaf_bootstrap_alliter,1,3) / sqrt(iterations_bootstrap);
