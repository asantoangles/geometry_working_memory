        ...
        for delay_i = 1:size(time_segments, 1)
            ...
            for sequence_length = [3 4 34]
                ...
                for perf_i = 1:length(performance)
                    ...
                    if ~isfile([path_results '/group_results/distance_allpairs/' func2str(fun_i) '/' performance{perf_i} '/design_matrix'...
                        ...
                        if ~isfolder([path_results '/group_results/distance_allpairs/' func2str(fun_i) '/' performance{perf_i} '/design_matrix'])
                            mkdir([path_results '/group_results/distance_allpairs/' func2str(fun_i) '/' performance{perf_i} '/design_matrix']);
                        end
                        ...
                        for sub_i = 1:length(subjects)
                            ...
                            if ranks == 3
                                ...
                                tmp = separability_allpairs.distance_by_separation{1};
                                tmp1 = triu(tmp); tmp1(tmp1 == 0) = [];
    
                                if stim_order > 1
    
                                    tmp = separability_allpairs.distance_by_separation{2};
                                    tmp2 = triu(tmp); tmp2(tmp2 == 0) = [];
                                ...
                                if stim_order > 2
                                    ...
                                    tmp = separability_allpairs.distance_by_separation{3};
                                    tmp3 = triu(tmp); tmp3(tmp3 == 0) = [];
                                ...
                                distance_delay = [tmp1 tmp2 tmp3 tmp4];
                        ...
                        % save data matrix
                        writetable(data, [path_results '/group_results/distance_allpairs/' func2str(fun_i) '/' performance{perf_i} '/design_matrix'...
                            '/design_matrix_stim_resolved_refined_' sequence_length_filename '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2)) '.txt']);
                    ...
                    if ~isfile([path_results '/group_results/volume/' func2str(fun_i) '/' performance{perf_i} '/design_matrix'...
                        ...
                        for sub_i = 1:length(subjects)
                            ...
                            if stim_order == 2
                                ...
                                if length(separability.distance_by_separation{1}) ~= 7 || ...
                                   length(separability.distance_by_separation{2}) ~= 7 
                                
                                    % Target length
                                    target_len = 7;
                                
                                    % Pad each vector with repeated mean values at the front
                                    padded_vectors = cellfun(@(v) [repmat(mean(v), 1, target_len - length(v)), v], ...
                                                             separability.distance_by_separation(1:2), 'UniformOutput', false);
                                
                                    % Concatenate padded vectors
                                    distance_delay = vertcat(padded_vectors{:});
                            ...
                            if stim_order == 3
                                ...
                                if length(separability.distance_by_separation{1}) ~= 7 || ...
                                   length(separability.distance_by_separation{2}) ~= 7 || ...
                                   length(separability.distance_by_separation{3}) ~= 7 
                                    ...
                                    % Pad each vector with repeated mean values at the front
                                    padded_vectors = cellfun(@(v) [repmat(mean(v), 1, target_len - length(v)), v], ...
                                                             separability.distance_by_separation(1:3), 'UniformOutput', false);
                            ...
                            if stim_order == 4
                                ...
                                if length(separability.distance_by_separation{1}) ~= 7 || ...
                                   length(separability.distance_by_separation{2}) ~= 7 || ...
                                   length(separability.distance_by_separation{3}) ~= 7 || ...
                                   length(separability.distance_by_separation{4}) ~= 7 
                                    ...
                                    % Pad each vector with repeated mean values at the front
                                    padded_vectors = cellfun(@(v) [repmat(mean(v), 1, target_len - length(v)), v], ...
                                                             separability.distance_by_separation(1:4), 'UniformOutput', false);
                ...
                for perf_i = 1:length(performance)
                    ...
                    if ~isfile([path_results '/group_results/principal_angle_min/' func2str(fun_i) '/' performance{perf_i} '/design_matrix'...
                        ...
                        for sub_i = 1:length(subjects)
                            ...
                            load([path_results '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i} ...
                                '/stim_resolved_refined_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))...
                                '/angle_min_bootstrap_' sequence_length_filename '.mat']); % angle_min_bootstrap
