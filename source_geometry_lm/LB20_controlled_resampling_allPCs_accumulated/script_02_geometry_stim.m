...
% loop over subspace dimensionalities
for last_comp_i = [6 8]
                    ...
                    for fun_i = functions_data
                        ...
                        for delay_i = 1:size(time_segments, 1)
                            ...
                            if ~isfile([path_results '/' subject_ID '/' func2str(fun_i) '/' performance{perf_i}...
                                    '/' events{event_i} '_' num2str(time_segments(delay_i,1)) 'to' num2str(time_segments(delay_i,2))...
                                    '/angle_min_perm_' sequence_length_filename '.mat'])
            
                                angle_perm = nan(stim_order,stim_order,permutations);
                                angle_min_perm = nan(stim_order,stim_order,permutations);
                                vaf_perm = nan(stim_order,stim_order,permutations);
                                euclidean_distances_perm = zeros(number_locations * stim_order, permutations);
                                ...
                                for perm_i = 1:permutations
                                    ...
                                    [angle_min_perm(:,:,perm_i), angle_perm(:,:,perm_i), vaf_perm(:,:,perm_i)] = compute_angle_vaf_accumulated(planes, stim_order);
                                    ...
                                    separability = [];
                                    separability.volume = cell(stim_order,1);
                                    separability.distance_by_separation = cell(stim_order,1);
